export async function getVideos({ token, data, params }) {
  const response = await fetch(`http://localhost:3001/video?${params}`, {
    method: "GET",
    headers: new Headers({
...
export async function explore({ token, data }) {
  const response = await fetch(`http://localhost:3001/video/explore`, {
    method: "POST",
    headers: new Headers({
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export async function getDownloadToken({ token }) {
  const response = await fetch(`http://localhost:3001/video/dl-token`, {
    method: "GET",
    headers: new Headers({
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export async function downloadVideo({ token, params, query }) {
  const response = await fetch(
    `http://localhost:3001/video/${params}/download${query}`,
    {
      method: "GET",
      headers: new Headers({
        "Content-Type": "application/json",
